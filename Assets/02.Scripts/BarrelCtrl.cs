...
public class BarrelCtrl : MonoBehaviour
{
    ...
    //수직 폭발력 
    public float forceV = 1500.0f;
    //수평 폭발력
    public float forceH = 800.0f;

    //Barrel의 Transform 컴포넌트를 저장할 변수
    private Transform tr;
    ...
    void ExpBarrel()
    {
        ...
        //지정한 원점을 중심으로 10.0f 반경 내에 들어와 있는 Barrel의 Collider 객체 추출
        Collider[] colls = Physics.OverlapSphere(tr.position, range, 1<<8);
        
        //추출한 Collider의 Rigidbody에 폭발력 전달
        foreach (Collider coll in colls)
        {
            Rigidbody rb = coll.GetComponent<Rigidbody>();
            rb?.AddExplosionForce(forceV, tr.position, 10.0f, forceH);
        }
